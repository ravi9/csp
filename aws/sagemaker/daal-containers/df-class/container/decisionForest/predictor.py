...
class ScoringService(object):
	...
	def get_model(cls):
		...
		if cls.model == None:
			with open(os.path.join(model_path, 'decision-forest-classifier-model.pkl'), 'rb') as inp:
				cls = pickle.load(inp)
		return cls.model
	...
	def predict(cls, input):
		...
		with open(param_path, "r") as pf:
			params = json.load(pf)			
			predict_algo=decision_forest_classification_prediction(int(params["nClasses"]),
...
def ping():
	...
	health = os.path.isfile(vectors_path) and os.path.isfile(param_path) # You can insert a health check here
